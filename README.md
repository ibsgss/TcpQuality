...
默认检测三网，每个节点发送 60 个包，并行数为 31，A/AAAA 解析使用 `223.5.5.5`：
...
脚本仅检测本机可用的 IP 协议；缺少 IPv4 或 IPv6 时会自动跳过对应节点。发送 TCP SYN 探测包通常需要使用 `root` 用户运行。域名解析固定使用 `223.5.5.5`，IPv4 会跳过私网、保留地址和 `198.18.0.0/15` 测试网段。
