...
// ipAPIsV4 are queried in order for the public IPv4 address. Endpoints that
// return a bare address over plain HTTPS come first.
var ipAPIsV4 = []string{
	"https://api.ipify.org",
...
// ipAPIsV6 are queried in order for the public IPv6 address.
var ipAPIsV6 = []string{
...
func Detect(ctx context.Context) Stack {
	...
	go func() {
		if ip, ok := detectFamily(ctx, "tcp4", ipAPIsV4, IsPublicIPv4); ok {
			s.IPv4, s.IPv4Work = ip, true
		}
		done <- struct{}{}
	}()
	go func() {
		if ip, ok := detectFamily(ctx, "tcp6", ipAPIsV6, IsValidIPv6); ok {
...
func detectFamily(ctx context.Context, network string, apis []string, valid func(string) bool) (string, bool) {
	...
	for _, api := range apis {
		...
		// Equivalent to curl -f: ignore error pages that would otherwise be
