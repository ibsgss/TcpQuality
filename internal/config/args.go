...
func (c *Config) ParseArgs(args []string) error {
	...
	for i = 0; i < len(args); i++ {
		...
		case "--intl":
			...
			c.InternationalEnabled = true
			if !c.ReportUploadForcedOff {
				c.UploadReport = true
			}
		case "--no-rank-upload":
			c.UploadReport = false
			c.ReportUploadForcedOff = true
		case "--debug":
			c.DebugMode = true
		case "--province":
			v, ok := next()
...
func (c *Config) applyDerived() {
	...
	if c.IntlRequested &&
		...
		!c.TestCernet && !c.TestAll && !c.IncludeDefaultRoute && !c.RouteMode &&
...
`, MaxPackets, DefaultPackets, MaxParallel)
}
