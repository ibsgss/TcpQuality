...
func (a *App) runInternationalMode(ctx context.Context) error {
	...
	if a.cfg.UploadReport {
		// No speedtest ran, so there is nothing to rank.
		a.rankDisabledReason = report.ReasonSpeedtestNotPerformed
		a.uploadReport(ctx, b.Bytes(), short, nil)
	}
	a.print("\n")
	return nil
}
...
// uploadReport posts the CSV and prints the resulting link or a warning. A
// non-nil session makes the run eligible for the speed ranking.
func (a *App) uploadReport(ctx context.Context, csv []byte, reportTime string, session *report.RankSession) {
	res, err := report.Upload(ctx, a.cfg.ReportAPI, csv, report.UploadOptions{
		ReportTime:         reportTime,
		PublicIPv4:         a.stack.IPv4,
		PublicIPv6:         a.stack.IPv6,
		Session:            session,
		RankDisabledReason: a.rankDisabledReason,
	})
	if err != nil {
	...
	if a.cfg.DebugMode && res.RankUpdated != nil && !*res.RankUpdated && res.RankRejectReason != "" {
		a.printf("  %s[!] 排名未更新：%s%s\n", config.Yellow, res.RankRejectReason, config.NC)
	}
}
