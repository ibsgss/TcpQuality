...
func (a *App) runMainMode(ctx context.Context) error {
	...
	var session *report.RankSession
	if a.cfg.SpeedtestEnabled {
		speedResults, session = a.runSpeedtest(ctx, "速度测试")
	} else {
		a.rankDisabledReason = report.ReasonSpeedtestNotPerformed
	}

	// ---- render + CSV + upload ----
	a.renderMain(ctx, st, res, labels4, labels6, labelsLarge, labelsEdu4, labelsEdu6, intlResults, speedResults, session)
	return nil
}
...
func (a *App) deriveState(set *nodes.Set) mainState {
	...
	testCDN := true
	// TCPQUALITY_INCLUDE_DEFAULT_ROUTE keeps the three-carrier CDN section
	// alongside --cernet instead of replacing it.
	if a.cfg.TestCernet && !a.cfg.TestAll && !a.cfg.IncludeDefaultRoute {
		testCDN = false
		st.normalCDN = false
		st.testEdu = true
...
func (a *App) largePacketPrecheck(ctx context.Context) bool {
	...
	// Heavy but incomplete loss also indicates large packets are being dropped,
	// so the threshold sits below 100%.
	return r.Status == probe.StatusOK && r.LossPct < config.LargePacketPrecheckMaxLoss
}
...
func (a *App) renderMain(ctx context.Context, st mainState, res struct {
	...
	intlResults []international.Result, speedResults *speedtest.Results, session *report.RankSession) {
	...
	// The large-packet block sits between IPv4 and IPv6 so both IPv4 sections
	// read together.
	if st.normalCDN {
		if st.ipv4Enabled {
			a.print(render.FamilyResults("IPv4回程", res.cdn4, labelFunc(labels4)))
		}
		if st.largeEnabled {
			a.print(render.LargePacketResults("IPv4大包回程", res.large4, labelFunc(labelsLarge), st.firewallLimited))
		}
		if st.ipv6Enabled {
			a.print(render.FamilyResults("IPv6回程", res.cdn6, labelFunc(labels6)))
		}
	}
	// Education sections are driven by what actually produced results, not by
	// which families were requested.
	if st.testEdu {
		hasV4, hasV6 := len(res.cernet) > 0, len(res.cernet2) > 0
		switch {
		case hasV4 && hasV6:
