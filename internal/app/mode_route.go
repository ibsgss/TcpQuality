...
func (a *App) fetchNodes(ctx context.Context, scope string) (*nodes.Set, error) {
	set, err := nodes.Fetch(ctx, a.cfg.GetNodesURL, scope)
	if err != nil {
		a.printf("%s[X] 无法从 getNodes 获取节点 IP+端口，请稍后重试%s\n", config.Red, config.NC)
		var fe *nodes.FetchError
		url := a.cfg.GetNodesURL
		if errors.As(err, &fe) {
			url = fe.URL
		}
		a.printf("%s    getNodes: %s%s\n", config.Dim, url, config.NC)
