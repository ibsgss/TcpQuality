...
// SpeedtestNode is a tosutil entry IP for one carrier in one TOS region.
...
// regionFromTarget derives the TOS region from the node's target column,
// defaulting to cn-beijing.
func regionFromTarget(target string) string {
	switch {
	case strings.Contains(target, "cn-shanghai"):
		return "cn-shanghai"
	case strings.Contains(target, "cn-guangzhou"):
		return "cn-guangzhou"
	default:
		return "cn-beijing"
	}
...
func Parse(r io.Reader) (*Set, error) {
	...
	for sc.Scan() {
		...
		typ, family := f[0], f[1]
		prov, isp, host, ip, port, target := f[2], f[3], f[4], f[5], f[6], f[7]
		...
		if family == "4" {
			...
			case "tos", "tosutil", "speedtest":
				if carrier, ok := normalizeCarrier(isp); ok {
					set.TOS = append(set.TOS, SpeedtestNode{
						ISP:    carrier,
						IP:     ip,
						City:   firstNonEmpty(prov, "北京"),
						Region: regionFromTarget(target),
					})
				}
			}
		}
...
// FetchError carries the request URL alongside the failure so the caller can
// print the same diagnostics as require_remote_nodes.
...
// newClient builds an HTTP client that either forces IPv4 or uses the system
// default address selection.
func newClient(forceV4 bool) *http.Client {
...
// Fetch downloads and parses the node feed for the given scope. It tries IPv4
// first (matching `curl -4`) and falls back to the default stack, so IPv6-only
// resolution problems do not abort the run. Failures are wrapped in *FetchError.
func Fetch(ctx context.Context, baseURL, scope string) (*Set, error) {
	url := buildURL(baseURL, scope)
	var firstErr error
	for _, forceV4 := range []bool{true, false} {
		set, err := fetchOnce(ctx, newClient(forceV4), url, scope)
		if err == nil {
			return set, nil
		}
		if firstErr == nil {
	...
	return nil, &FetchError{URL: url, Err: firstErr}
}

func fetchOnce(ctx context.Context, client *http.Client, url, scope string) (*Set, error) {
