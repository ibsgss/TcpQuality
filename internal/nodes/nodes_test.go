...
const sampleTSV = "type\tfamily\tprov\tisp\thost\tip\tport\ttarget\tbackup_host\tbackup_ip\tbackup_port\tbackup_target\n" +
	...
	"tos\t4\t上海\t电信\t\t9.9.9.9\t\ttos-cn-shanghai.volces.com\t\t\t\t\n" +
	"junk\t4\t\t\t\t\t\t\t\t\t\t\n" +
...
func TestParse(t *testing.T) {
	...
	if len(set.TOS) != 3 {
		t.Fatalf("TOS count = %d, want 3", len(set.TOS))
	}
	if set.TOS[0].ISP != "电信" || set.TOS[0].City != "北京" || set.TOS[0].Region != "cn-beijing" {
		t.Errorf("TOS[0] wrong: %+v", set.TOS[0])
	}
	if set.TOS[1].ISP != "联通" {
		t.Errorf("TOS[1] carrier normalization wrong: %+v", set.TOS[1])
	}
	// The region comes from the target column, not the province name.
	if set.TOS[2].Region != "cn-shanghai" {
		t.Errorf("TOS[2] region = %q, want cn-shanghai", set.TOS[2].Region)
	}
}
...
func TestRegionFromTarget(t *testing.T) {
	...
	for in, want := range cases {
		if got := regionFromTarget(in); got != want {
			t.Errorf("regionFromTarget(%q) = %q, want %q", in, got, want)
		}
	}
}
