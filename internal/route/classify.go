...
// isUnicomRouteHop reports whether hop h belongs to Unicom, counting access
// ASNs as well as the backbone (mirrors is_unicom_route_hop).
func (t *trace) isUnicomRouteHop(h int) bool {
	return isUnicomBackboneASN(t.asns[h]) || isUnicomBackboneIP(t.ips[h]) || isUnicomAccessASN(t.asns[h])
}

// compactComboLabel collapses a multi-arrow label to its first and last leg.
func compactComboLabel(label string) string {
...
// mobileLabelBefore describes the China Mobile transit seen strictly before hop
// `last`, used to label cross-carrier Unicom routes (mirrors mobile_label_before).
func (t *trace) mobileLabelBefore(last int) string {
	if last <= 1 {
		return ""
	}
	hasCMIN2, hasCMI := false, false
	for h := 1; h < last; h++ {
		if isCMIN2ASN(t.asns[h]) {
			hasCMIN2 = true
		}
		if isCMIASN(t.asns[h]) ||
			(t.targetISP == "移动" && (isMobileAccessASN(t.asns[h]) || isMobileAccessIP(t.ips[h]))) {
			hasCMI = true
		}
	}
	switch {
	case hasCMIN2 && hasCMI:
		return "CMIN2->CMI"
	case hasCMIN2:
		return "CMIN2"
	case hasCMI:
...
// has163Before reports whether a China Telecom 163 hop appears strictly before
// hop `last`.
func (t *trace) has163Before(last int) bool {
	if last <= 1 {
		return false
	}
	for h := 1; h < last; h++ {
		if t.asns[h] == "4134" || t.asns[h] == "4847" || is163IP(t.ips[h]) {
			return true
		}
	}
	return false
...
func (t *trace) unicomRouteComboLabel() string {
	...
	for h := 1; h <= t.maxHop; h++ {
		...
		if t.isUnicomRouteHop(h) {
			firstUnicom = h
			break
		}
	}
	domestic := t.unicomDomesticLabelFromHop(firstUnicom - 1)
	if t.targetISP == "联通" && domestic != "" {
		if mobileTransit := t.mobileLabelBefore(firstUnicom); mobileTransit != "" {
			return compactComboLabel(mobileTransit + "->" + domestic)
		}
		if t.has163Before(firstUnicom) {
			return "163->" + domestic
		}
	}
	return domestic
...
func (t *trace) labelFromTargetIP() string {
	...
	case t.targetISP == "移动" && isCMIN2ASN(asn):
		return "CMIN2"
	case t.targetISP == "移动" && (isCMIASN(asn) || isMobileAccessASN(asn) || isMobileAccessIP(t.destIP)):
		return "CMI"
	}
	return ""
}
...
func (t *trace) classify() string {
	...
	// CMIN2 outranks the per-hop backbone scan so a CMIN2 transit is not
	// reported as plain CMI.
	if t.hasASN("58807") {
		return "CMIN2"
	}
	for hop := 1; hop <= t.maxHop; hop++ {
